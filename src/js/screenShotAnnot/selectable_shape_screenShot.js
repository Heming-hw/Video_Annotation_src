...
module.exports = class SelectableShape_screenShot {
  constructor(imgDiv) {
    this.$parent = imgDiv

    // var screenImag = $(".vac-video-cover")
    // this.$parent = screenImag
  ...
  bindEvents() {
    ...
    this.$parent.on('mousedown.vac-selectable-shape', e => {
      ...
      if (!e.delegateTarget.querySelector(".sac-cover-canvas")) return; // didn't click on overlay
      if (e.delegateTarget.querySelector(".vac-shape")) return; // user clicked on annotation

      // Remove old shape if one existe
  ...
  render() {
    ...
    this.$el = $('<div/>').addClass('sac-shape');
    this.setDimsFromShape();
    // this.$parent.append(this.$el);
    $(".sac-cover-canvas").append(this.$el)
  }
